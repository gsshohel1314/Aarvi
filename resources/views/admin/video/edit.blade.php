          ...
          <div class="card">
              ...
              <div class="card-body card_form">
                ...
                <div class="form-group row custom_form_group{{ $errors->has('cate_id') ? ' has-error' : '' }}">
                    <label class="col-sm-3 control-label">Video Category:<span class="req_star">*</span></label>
                    <div class="col-sm-7">
                      <select class="form-control" name="cate_id">
                          <option value="">Select Video Category</option>
                          @foreach($videoCategories as $value)
                            <option value="{{$value->id}}" @if($value->id==$data->cate_id) selected @endif >{{$value->name}}</option>
                          @endforeach
                      </select>
                ...
                <div class="form-group row custom_form_group">
                    ...
                    <div class="col-sm-2">
                        @if($data->image!='')
                            <img class="img-thumbnail" src="{{asset('uploads/video/'.$data->image)}}" alt="photo"/>
                        @else
